...
async def link_share_delete(client, query):
    ...
    buttons = [[InlineKeyboardButton(f"🔴 {data.get('name', cid)}", callback_data=f"ls_del:{cid}")] for cid, data in channels.items()]
...
async def send_link_share_page(client, query, request_link: bool, page: int):
    ...
    for cid, data in chunk:
        ...
        row.append(InlineKeyboardButton(f"🔵 {data.get('name', str(channel_id))}", url=link))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
