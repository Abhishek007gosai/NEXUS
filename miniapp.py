...
from plugins.sources import SOURCES

# Optional: Pyrogram bot instance injected at runtime for log-channel notifies
pyro_bot = None  # set by bot.py after start
...
# Notifications use Telegram Bot HTTP API (sync) from Flask threads.
...
def _bot_api(method: str, payload: dict):
    """Fire-and-forget Telegram Bot API call (sync)."""
...
def notify_new_report(title: str, reason: str, details: str, reporter_name: str):
    ...
    _bot_api("sendMessage", {"chat_id": Config.LOG_CHANNEL_ID, "text": body})
...
def _normalize_join_link_async(raw: str) -> str:
    ...
    if re.fullmatch(r"-?\d+", raw):
        result = _bot_api("createChatInviteLink", {"chat_id": int(raw)})
        if result and result.get("ok") and result.get("result", {}).get("invite_link"):
...
def normalize_join_link(raw: str) -> str:
    return _normalize_join_link_async(raw)
